    ...
    function render_notice($name)
    {
        ...
        if (($handler = $this->get_handler_instance($name)) && !$this->is_dismissed($name)) {
            ...
            if ($has_method && $handler->is_renderable() || !$has_method) {
                //next line fix -> PHP Fatal error:  Unparenthesized `a ? b : c ? d : e` is not supported. Use either `(a ? b : c) ? d : e` or `a ? b : (c ? d : e)` in /var/www/vhosts/mccm-feldkirch.at/httpdocs/wp-content/plugins/nextgen-gallery/products/photocrati_nextgen/modules/nextgen_admin/package.module.nextgen_admin.php on line 368
